...
-- ---------------------------------------------------------------------
-- Start INIT From Hasura GraphQL Engine
-- ---------------------------------------------------------------------
...
-- Hasura timestamp updated function
...
-- Hasura timestamp updated function
CREATE FUNCTION public.ensure_slugify ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    _new := NEW;
    IF LENGTH(_new."slug") = 0 THEN
        _new."slug" = public.slugify(_new."name");
    ELSE
        _new."slug" = public.slugify(_new."slug");
    END IF;
    RETURN _new;
END;
$$;
